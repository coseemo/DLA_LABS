...
def experiment_1_3(config):
    ...
    for depth in config["experiment"]["depths"]:
        for residual in [False, True]:
            model_config = config["model"].copy()
            model_config["params"]["layers"] = depth
            model_config["params"]["residual"] = residual
...
def experiment_2_2(config):
    ...
    teacher_runner.setup_training(
        criterion=config["training_teacher"]["criterion"],
        optimizer=config["training_teacher"]["optimizer"],
        lr=config["training_teacher"]["lr"],
        scheduler=config["training_teacher"]["scheduler"]
    )
    
    #Osserviamo le performance del teacher
    teacher_runner.train(train_dl, val_dl, config["training_teacher"]["epochs"])
    ...
    student_runner.setup_training(
        criterion=config["training_student"]["criterion"],
        optimizer=config["training_student"]["optimizer"],
        lr=config["training_student"]["lr"],
        scheduler=config["training_student"]["scheduler"],
        max_iter=config["training_student"]["epochs"]
    )
    
    #Osserviamo le performance dello studente senza distillazione
    student_runner.train(train_dl, val_dl, config["training_student"]["epochs"])
    ...
    distillery.setup_optimizer(
        optimizer_class=getattr(optim, config["training_student"]["optimizer"]),
        lr=config["training_student"]["lr"]
    )
    ...
    distill_dl = data_loader.create_distillation_dataloader(distill_train_dl, teacher_logits)
    
    #Attuiamo la distillazione
    distill_losses, distill_accs = distillery.distillation(
        distill_dl, config["training_student"]["epochs"]
    )
    ...
    #Inforkmazioni sulla differenza di accuracy 
    accuracy_gain = distilled_test_accs[0] - student_test_accs[0]
    print(f"Accuracy gain from distillation: {accuracy_gain:.4f} (+{accuracy_gain*100:.2f}%)")

    distill_logger.log_metrics({
        "teacher_accuracy": teacher_test_accs[0],
        "student_baseline_accuracy": student_test_accs[0],
        "distilled_student_accuracy": distilled_test_accs[0],
    ...
    "parameter_reduction_pct": (1 - student_params/teacher_params)*100
    })
